...
(function () {
    ...
    function redheadsound(component, _object, prefer_dash) {
      ...
      function getPage(card) {
        ...
        network["native"](component.proxyLink(url, prox), function (str) {
          str = (str || '').replace(/\n/g, '');
          var embedMatch = str.match(/https:\/\/kinescope\.io\/embed\/[a-zA-Z0-9]+[^"'\s]*/);
          var player_link = embedMatch ? embedMatch[0] : null;

          // 2. Если не нашли прямым поиском, ищем в скриптах
          if (!player_link) {
              var scriptRegex = /<script[^>]*>([\s\S]*?)<\/script>/gi;
              var match;
              while ((match = scriptRegex.exec(str)) !== null) {
                 var scriptContent = match[1];
                  if (scriptContent.includes('kinescope') || scriptContent.includes('Kinescope')) {
                      var urlMatch = scriptContent.match(/https:\/\/kinescope\.io\/embed\/[^"'\s]+/);
                      if (urlMatch) {
                          player_link = urlMatch[0];
                          break;
                      }
                  }
              }
          ...
          if (player_link) {
              console.log('🎬 Found Kinescope embed:', player_link);

              // 3. Загружаем страницу Kinescope
              network.clear();
              network.timeout(10000);
              network.native(component.proxyLink(player_link, prox, prox_enc), function (embedStr) {
                  // 4. Ищем playerOptions в ответе Kinescope
                  parse(embedStr, player_link);
              }, function (a, c) {
                  component.empty(network.errorDecode(a, c));
              }, false, {
                  dataType: 'text',
              ...
              console.log('❌ No Kinescope embed URL found');
              component.emptyForQuery(select_title);
          }
      }, function (a, c) {
          component.empty(network.errorDecode(a, c));
      ...
      function parse(str, url) {
        component.loading(false);
        str = (str || '').replace(/\n/g, '');
        var find = str.match(/var playerOptions = ({[\s\S]*?});/);
        var json;

        if (find && find[1]) {
            try {
                // Пробуем распарсить как есть
                json = JSON.parse(find[1]);
            } catch (e) {
                // Если не получается, пробуем исправить JSON
                try {
                    var fixed = find[1]
                        .replace(/\\u0026/g, '&')     // Unicode & -> &
                        ...
                        .replace(/([{,]\s*)(\w+)(\s*:)/g, '$1"$2"$3'); // Ключи без кавычек

                    json = JSON.parse(fixed);
                    console.log('✅ JSON parsed after fixing');
                } catch (e2) {
                    console.warn('⚠️ Could not parse playerOptions:', e2.message);
                    console.log('Raw data sample:', find[1].substring(0, 200));
                }
            }
        }
